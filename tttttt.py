# -*- coding: utf-8 -*-
"""
Created on Thu Mar  9 19:41:50 2017
...
def set_initial_values(state_num, good_states, bad_states, tie_states):
	...
	bad_value = -1000
	tie_value = 100
	for i in range(0, 19683):						# Considering all 3^9 states
...
def convert_state_to_decimal(state):
	...
	for k in range(0, 9):
		i =int(( k/3))
		j = int((k%3))
		decimal_value =decimal_value+ three_pow * state[i][j]
		three_pow /= 3
	return decimal_value 

def user_input():
	""" Returns False for continue, True for exit """
	input_str = input('Enter \'X\' to exit, Enter / any other string to continue: ')
	return input_str == 'X'
...
def main():
	...
	try:
		...
		choice = input('Load previous progress - Y\~Y : ')	
		if choice == 'Y':
	...
	while(True):
		...
		for state in goto_states:
			temp_max = state_num[int(convert_state_to_decimal(state))]
		...
		state_num_current_state = learning_rate*state_num[int(convert_state_to_decimal(goto_state))]
		state_num[int(convert_state_to_decimal(current_state))] = state_num_current_state
		...
		print (t)
		t.clear()
		temp = learning_rate*state_num[int(convert_state_to_decimal(current_state))]
		state_num[int(convert_state_to_decimal(prev_state))] = temp
